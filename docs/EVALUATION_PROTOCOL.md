...
重なりのあるwindowの長さを足すと観測時間を過大評価するため、時間換算には
`--window-stride-seconds` の明示も必須です。6.4秒窓・1.6秒strideを6.4秒として
換算するとwindow誤判定数/時間を4分の1に見せてしまいます。
この換算も等間隔の負例stride当たりのwindow判定数であり、連続誤報をまとめたevent数ではありません。
不規則な間隔や記録の欠落があるデータには使わず、実際の時刻と観測区間から評価します。
