...
2026-09-06訂正：実データは存在しないとのユーザー確認を受けた。本書で監査した配列の
出自は未確認であり、実測データの評価結果とは扱わない。

## 追加監査

時間方向の共有を調べ、元の分割が独立評価になっていないことを確認した。
最新の分割修正・診断結果・ラベル境界の扱いは [転倒ラベル評価の監査・修正・比較](REAL_FALL_EVALUATION.md) を参照。
ラベル照合に合格するだけでは正式評価の準備完了と判定しない。
以下の「不整合」は照合ルールとの不一致を指し、元ラベルが誤りと確定した意味ではない。
