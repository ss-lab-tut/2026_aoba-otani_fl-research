...
この1件をどちらのラベルへ合わせるべきかは、元データの作成根拠を確認しなければ決められません。正式評価では、先生またはデータ作成者と正解ラベルを確認し、修正版データを新しいchecksumで固定します。それまではB1/B2の正式値を確定しません。
...
現在の配列には施設名、設置条件、遮蔽条件を対応付けるmetadataがありません。そのため、既存の検証データだけでは先生が指定した条件別評価へ分割できません。最低限、各sampleに次のいずれかを対応付ける必要があります。
