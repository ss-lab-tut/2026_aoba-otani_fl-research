# B1/B2/B3/P1：比較実装・5 seeds実行・結果整理の完了報告（2026-09-08）

2026-09-14更新：[新規軌跡・6/18 epochsの追加比較](FRESH_TRAJECTORY_RESULTS.md)を完了した。
以下は9月8日時点の主解析の記録として保持する。

**比較実装・5 seeds実行・結果整理は完了した。**
今回の合成データ条件では、P1はB3より誤検知率を下げたが、平均Recall/F1の改善は確認できなかった。
本報告では、比較方法、実施内容、結果と限界、再現手順をまとめる。
論文の主張と追加実験の必要性は、この結果をもとに次回レビューで検討する。

## 比較の目的と対象範囲

国際会議への投稿準備として、新しいアイデアの追加より既存内容の論文化を優先する。
9月8日頃までにB1/B2/B3/P1を同じ条件で比較できる状態にし、とくに施設別thresholdのB3と、
condition estimatorにより動的にthresholdを変えるP1の比較を重点とした。
5 seedsを一括実行し、施設・geometry別指標とunseen geometryの結果をCSVにまとめた。
...
| 5 seedsを一括実行 | 完了。0〜4の5 model seeds、計10 CNN checkpointsを検証 |
...
| condition estimatorを固定 | 完了。固定artifactのhash一致を確認して使用 |
| fall detectionへ接続 | 完了。観測→既存CNNと条件推定→threshold→判定。実測ではなく合成データの指定時点評価 |
| P2・新CNN・GAN | 今回の比較対象外 |
| 9月9日頃の結果レビュー | レビュー資料を準備済み |
...
結果レビューで追加実験が必要となった場合は、既存CNNの学習安定性や調整条件に範囲を絞り、
...
比較結果と再現手順を保存し、次回レビューで論文の方向性と追加実験を検討できる状態に整理した。
