...
def evaluate_tradeoff(
    ...
    *,
    ...
    supplied together with an explicit stride. Duration alone cannot establish
    exposure when windows overlap. The rate counts positive windows per hour
    of negative-labelled strides; it is not an event alarm rate.
    """
    ...
    if window_seconds is not None:
        ...
        if window_stride_seconds is None:
            raise ValueError("explicit window_stride_seconds required for hourly window rates")
    if window_stride_seconds is not None:
        if window_seconds is None or not np.isfinite(window_stride_seconds) or not 0 < window_stride_seconds <= window_seconds:
            raise ValueError("require 0 < window_stride_seconds <= window_seconds")
    ...
    for threshold in threshold_values:
        ...
        for condition, mask in groups:
            ...
            if window_seconds is not None:
                negative_hours = negative_count * window_stride_seconds / 3600.0
