...
class EvaluateTradeoffTests(unittest.TestCase):
    ...
    def test_overlapping_windows_need_stride_for_hourly_rate(self):
        with self.assertRaisesRegex(ValueError,'stride'):
            evaluate_tradeoff([0,1],[.8,.9],window_seconds=6.4)
        row=evaluate_tradeoff([0,1],[.8,.9],thresholds=[.5],window_seconds=6.4,window_stride_seconds=1.6)[0]
        self.assertEqual(row['false_alerts_per_hour'],2250.)
